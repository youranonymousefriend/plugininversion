...
from model import get_default_robust, get_default_model
...
def main():
    ...
    target = args.target
    saver = ExperimentSaver(f'NatZoomX{target}', save_id=True, disk_saver=True)
    batch_size = 32

    model = get_default_model().cuda()
    ...
    for i in range(32, mx_size + 1, 32):
        image = new_init(mx_size, last=image, padding=0)
        post = torch.nn.Sequential(Zoom(out_size=mx_size), Jitter(), ColorJitter(batch_size, shuffle_every=True))

        visualizer = ImageNetVisualizer(loss, saver, post, pre, print_every=10, lr=0.1, steps=50, save_every=50)
