...
def main():
    ...
    target = args.target
    saver = ExperimentSaver(f'NatCenterX{target}', save_id=True, disk_saver=True)
    batch_size = 32
    ...
    for i in range(32, mx_size + 1, 32):
        post = torch.nn.Sequential(Focus(mx_size, 0), Jitter(), ColorJitter(batch_size, shuffle_every=True))
