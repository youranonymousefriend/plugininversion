from augmentation import Clip, Jitter, Focus, ColorJitterR, RepeatBatch
from inversion import ImageNetVisualizer
from loss import LossArray, TotalVariation, CrossEntropyLoss, ViTCrossEntropyLoss
from model import get_default_robust, get_robust_normal, get_default_vit, get_default_model
...
def main():
    ...
    target = args.target
    saver = ExperimentSaver(f'Develop{target}', save_id=True, disk_saver=True)
    batch_size = 32
    steps = 50 
    ...
    for i in range(32, mx_size + 1, 32):
        post = torch.nn.Sequential(Focus(i, 0), Jitter(), ColorJitterR(batch_size, shuffle_every=True))
