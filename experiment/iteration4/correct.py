...
def main():
    ...
    target = args.target
    saver = ExperimentSaver(f'Correct{target}', save_id=True, disk_saver=True)
    batch_size = 32
    mx_size = 256
    ...
    for i in range(4*32, mx_size + 1, 32):
        if last is None:
            down = torch.nn.Upsample(size=(i, i), mode='bilinear', align_corners=False).cuda()
            image = down(inv_dataset[target])
        else:
            up = torch.nn.Upsample(size=(i, i), mode='bilinear', align_corners=False).cuda()
            image = up(last).cuda()
