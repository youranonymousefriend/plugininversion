...
def main():
    ...
    target = args.target
    saver = ExperimentSaver(f'Robust{target}', save_id=True, disk_saver=True)
    batch_size = 32
    mx_size = 256
    ...
    for i in range(2 * 32, mx_size + 1, 32):
        ...
        visualizer = ImageNetVisualizer(loss, saver, pre, post2, print_every=10, lr=0.1, steps=300, save_every=300)
