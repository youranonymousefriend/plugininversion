...
def main():
    ...
    constructor, batch_size, name, image_size = get_default_vit, 4, 'Vit_v2', 384
    ...
    step = 48
    padding = [0, step // 2, step]
    pad = padding[way]

    for i in range(step, image_size + 1, step):
        image = new_init(i, last=image, padding=pad)
        seq = [Focus(i, 0), Zoom(out_size=image_size), Jitter(), RepeatBatch(batch_size),
