...
def main():
    ...
    steps = 300
    saver = ExperimentSaver(f'NatBatchSize{target}{batch_size}', save_id=True, disk_saver=True)
    ...
    for i in range(32, mx_size + 1, 32):
        post = torch.nn.Sequential(Focus(i, 0), Jitter(), ColorJitter(batch_size, shuffle_every=True))
