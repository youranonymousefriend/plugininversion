...
from .activation import AbstractMiniBatchActivation


class BatchNormHook(AbstractMiniBatchActivation):
    def hook_fn(self, model: nn.Module, x: torch.tensor):
        raise NotImplementedError

    @staticmethod
    def get_mean_var(x: torch.tensor) -> (torch.tensor, torch.tensor):
        view = x.transpose(1, 0).contiguous().view([x.size(1), -1]).to('cuda:0')
...
class MatchModelBNStatsHook(BatchNormHook):
